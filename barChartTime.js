fetch('someDocuments.json')
  ...
  .then(jsonData => {
    ...
    // Initialize an object to store time spent in each category
    const categoryStats = {};
    ...
    // Calculate total time spent for each category
    ...
    const processedData = categories.map(category => {
      return {
        "category": category,
        "timeActive": categoryStats[category].timeActive // Time spent in seconds
      };
    });
    ...
    const spec = {
      ...
      "description": "Bar chart showing time spent per category",
      "width": 800,
      "height": 400,
      ...
      "encoding": {
        ...
        "y": { "field": "timeActive", "type": "quantitative", "axis": { "title": "Time Spent (seconds)" } },  // Y-axis as time spent
        "color": {
          "field": "category",
          "type": "nominal",
          "scale": {
